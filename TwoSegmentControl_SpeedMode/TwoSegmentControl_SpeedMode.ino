...
#define LIMIT_CURRENT  400 //モータの電流値がこの値に制限される（壁との接触時に指定したトルクで押しつけ可能）
#define JOINT_GOAL_POSITION 2048  //関節モータの中心位置

const uint8_t FIRST_SEGMENT_SHADOW_ID = 1;
const uint8_t SECOND_SEGMENT_SHADOW_ID = 4;
const uint8_t FIRST_JOINT_PITCH = 101;
const uint8_t FIRST_JOINT_YAW = 102;
...
void setup() {
  ...
  //関節の初期設定
  dxl.setGoalPosition(FIRST_JOINT_PITCH, JOINT_GOAL_POSITION);
  dxl.setGoalCurrent(FIRST_JOINT_PITCH, 50 );
  dxl.setGoalPosition(FIRST_JOINT_YAW, JOINT_GOAL_POSITION);
  dxl.setGoalCurrent(FIRST_JOINT_YAW, 50 );
  //dxl.torqueOff(FIRST_JOINT_PITCH);
  //dxl.torqueOff(FIRST_JOINT_YAW);

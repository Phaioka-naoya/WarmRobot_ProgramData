...
void setup() {
  ...
  dxl.writeControlTableItem(PROFILE_ACCELERATION, DXL_ID, 50);    //最大化加速度が右の値に制限　Acceleration [rpm²] = Value * 214.577
  dxl.writeControlTableItem(PROFILE_VELOCITY, DXL_ID, 200);       //最大速度が右の値に制限　Velocity [rpm] = Value * 0.229 [rpm]
  dxl.torqueOn(DXL_ID);
}
...
void loop() {
  ...
  dxl.setGoalPosition(DXL_ID, 1024);
